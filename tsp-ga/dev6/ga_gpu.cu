...
extern "C" __global__ void selection_child_kernel(World* old_pop, 
		int pop_size, 
		float* rand_nums,  // prob_select_d
		int* sel_ix,
		World *new_pop, 
		float prob_crossover, 
		float *prob_cross,  // prob_cross_d
		int *cross_loc,     // cross_loc_d
		float prob_mutation, 
		float *prob_mutate,  // prob_mutate_d
		int *mutate_loc, 
		const int seed, 
		...
		float *fit_sum)
{
	...
	{
		float distance = 0.f; // Total "normalized" "distance"
		int city_num = new_pop[tid].num_cities - 1;
		for (int i=0; i<city_num; i++) {
			float loc_x = new_pop[tid].cities[i].x - new_pop[tid].cities[i + 1].x;
			float loc_y = new_pop[tid].cities[i].y - new_pop[tid].cities[i + 1].y;
			distance += loc_x * loc_x + loc_y * loc_y;
		}

		new_pop[tid].fitness = (new_pop[tid].width * new_pop[tid].height) / distance;
	}
	...
	for (int tid = grid.thread_rank(); tid < pop_size; tid += grid.size()) {
		float sum = 0.f;
		for (int i=0; i<=tid; i++)
			sum += new_pop[i].fitness;
		new_pop[tid].fit_prob = sum;
		if (tid == (pop_size - 1)) {
			*fit_sum = sum;
		}
	}
	...
	for (int tid = grid.thread_rank(); tid < pop_size; tid += grid.size()) {
		new_pop[tid].fit_prob /= fit_sum[0];
	}
}
		...
		int blk_size, int grid_size, int pop_bytes, int numSms)
{
	...
	cout << "[g_execute] => pop_size " << pop_size  << endl;
    ...
    // Allocate fitness sum on the GPU
	float *fitsum_d;                                                           
	if (checkForError(cudaMalloc((void**) &fitsum_d, sizeof(float))))
		return true;
	...
	for (uint i=0; i<max_gen; i++)
	{
		...
		/*
		checkCudaErrors(cudaMemcpy(tmp, new_pop_d, sizeof(World), cudaMemcpyDeviceToHost));
		for(int ii=0; ii<10; ii++)                                                     
			cout << tmp[ii].fit_prob << endl;                                        
		cout << endl;                                                               
		*/
		...
		// Hint: this could use unified memory for optimization
		sel = g_select_leader(old_pop_d, pop_size, generation_leader,best_leader, Block, Grid);
