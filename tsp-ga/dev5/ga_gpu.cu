		...
		const int num_cities)
{
	...
	if (tid < pop_size)
	{
		...
		rand_nums[tid + tid]     = curand_uniform(&state);
		rand_nums[tid + tid + 1] = curand_uniform(&state);
		prob_cross[tid]          = curand_uniform(&state);
		prob_mutate[tid]         = curand_uniform(&state);
		cross_loc[tid]           = (int)(curand_uniform(&state) * (num_cities - 1));
