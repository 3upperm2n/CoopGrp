...
/*
__device__ void fit_prob_kernel_(World* pop, int pop_size, float* fit_sum, const cg::grid_group &grid)
{
	//if (tid < pop_size)
...
extern "C" __global__ void fitness_kernel(World* pop, int pop_size, float *fit_sum)
{
	...
	//fit_prob_kernel_(pop, pop_size, fit_sum, grid);
...
bool g_evaluate(World *pop, int pop_size, dim3 Block, dim3 Grid, int blk_size, int grid_size)
{
	...
	fit_sum_kernel <<< grid_size , blk_size >>> (pop, pop_size, fit_sum_d);
	if (checkForKernelError("fit_sum_kernel failing ")) return true;

	// Compute the full probabilities
	fit_prob_kernel <<< grid_size, blk_size >>> (pop, pop_size, fit_sum_d);
...
//	checkCudaErrors(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocksPerSm, fitness_kernel, 
//				numThreads, sMemSize));
...
//
	...
	//checkCudaErrors(cudaLaunchCooperativeKernel((void *)fitness_kernel, Grid, Block, kernelArgs, sMemSize, NULL));
		...
		int blk_size, int grid_size, int grid_size2, int pop_bytes)
{
	...
	// Calculate the fitnesses
	error = g_evaluate(old_pop_d, pop_size, Block, Grid, blk_size, grid_size);
	if (error) return true;
